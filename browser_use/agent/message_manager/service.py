...
class MessageManager:
    def __init__(
        ...
        system_prompt_class: Type[SystemPrompt] = None,
        max_input_tokens: int = 128000,
        estimated_characters_per_token: int = 3,
        image_tokens: int = 800,
    ...
    def add_new_task(self, new_task: str) -> None:
        ...
        # Update task if using new style
    ...
    def add_state_message(
        ...
        # Handle history style based on initialization pattern
        if hasattr(self, 'settings'):
            # New style
                ...
                for r in result:
                    if r.include_in_memory:
                        ...
                        if r.error:
                            ...
                            self._add_message_with_tokens(msg)
                        result = None  # if result in history, we don't want to add it again
            ...
            # Old style
            # Similar implementation with old variable names
            if result:
                for r in result:
                    if r.include_in_memory:
                        if r.extracted_content:
        ...
        for i, msg in enumerate(messages):
            if isinstance(msg, class_to_merge):
                if buffer_items > 0 and i < len(messages) - 1 and isinstance(messages[i+1], class_to_merge):
                    ...
                    # Flush buffer and add this message
                    buffer += "\n\n" + msg.content
                    merged_messages.append(class_to_merge(content=buffer))
                    buffer = ""
                    buffer_items = 0
                else:
                    # First message of its kind
